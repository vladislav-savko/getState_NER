...
if __name__ == "__main__":
    ...
    # Теперь код, который использует GPU, может безопасно работать с многозадачностью.
...
class BertClassifier(LabelStudioMLBase):
    ...
    def fit(self, event, data, **additional_params):
            ...
            for annotation in task['annotations']:
                if 'result' in annotation:
                    for result in annotation['result']:
                        if result['from_name'] == "taxonomy":
                            ds_raw['id'].append(task['id'])
                            text = self.preload_task_data(task, task['data'][value])
                            ds_raw['text'].append(text)
                            # ds_raw['label'].append(result['value']['taxonomy'])
                            unique_taxonomy_values = set()
                            for taxonomy_item in result['value']['taxonomy']:
                                unique_taxonomy_values.update(taxonomy_item)
                            # Преобразуем set обратно в список и добавляем в ds_raw['label']
                            ds_raw['labels'].append(list(unique_taxonomy_values))
        ...
        def label_to_id_function(examples):
            ...
            print(examples)
            # examples["label"] = [[category, label_to_id[value]] for category, value in examples["label"]]
            # examples["label"] = [label_to_id[f"{label[0]}/{label[1]}"] for label in examples["label"]]
            # examples["label"] = [[label_to_id[label] for label in label_list] for label_list in examples["label"]]
            return examples
        ...
        config = AutoConfig.from_pretrained(self.baseline_model_name, num_labels=len(labels), problem_type="multi_label_classification", ignore_mismatched_sizes=True)
